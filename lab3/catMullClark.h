...
int totFace=0;
vector<pointStruct> allVertices;
...
struct catMullClark {	
	...
	void  catMullClarkSurf(MyMesh mesh)
	{
		...
		for (MyMesh::FaceIter ff_it= mesh.faces_begin(); ff_it!=mesh.faces_end(); ++ff_it) 
		{
			...
			fdata.f.fh=fh;
			allVertices.push_back(fdata.f.facepoint);
			//cout<<"face pt"<<endl;
		...
		for (MyMesh::VertexIter v_it=mesh.vertices_begin(); v_it!=mesh.vertices_end(); ++v_it) 
		{
			...
			newVertex = Q/numOfPoints + (R+R)/numOfPoints + (mesh.point(v_it)/numOfPoints)*(numOfPoints-3);			
			pointStruct p;
			p.point = newVertex;
			p.index = counter;
	...
	//catMullClarkSurf func ends here
	...
	void createOutMesh(map<FaceHandle,FaceData> completeFaceData,MyMesh mesh)
	{
		MyMesh out;
		ofstream fout;
		fout.open("cClark.off",ios::out);

		int totFace=0;

		for(map<FaceHandle,FaceData> :: iterator mapit = completeFaceData.begin(); mapit != completeFaceData.end(); mapit++)
		{
			totFace = totFace + (*mapit).second.edgePointHash.size();
		}

		fout<<"OFF"<<endl;
		fout<<allVertices.size()<<"\t"<<totFace<<"\t"<<0<<endl;

		for(vector<pointStruct> :: iterator vi = allVertices.begin(); vi < allVertices.end(); vi++)
		{
			fout<<vi->point<<endl;
		}
